...
import com.simplation.androiddemos.function_summary.ifkey_sdk.speech.setting.IatSettings;
import com.simplation.androiddemos.function_summary.ifkey_sdk.speech.util.FucUtil;
import com.simplation.androiddemos.function_summary.ifkey_sdk.speech.util.JsonParser;
...
public class IatActivity extends BaseActivity implements View.OnClickListener {
    ...
    private void initPermissions() {
        rxPermissions.request(Manifest.permission.READ_CONTACTS, Manifest.permission.RECORD_AUDIO)
                .subscribe(aBoolean -> {
                    if (aBoolean) {
                        Toast.makeText(IatActivity.this, "已获得授权", Toast.LENGTH_SHORT).show();
                    } else {
                        Toast.makeText(IatActivity.this, "没有给予权限,请手动去设置", Toast.LENGTH_SHORT).show();
                    }
                });
     ...
     * 上传联系人/词表监听器。
     */
    private LexiconListener mLexiconListener = (lexiconId, error) -> {
        if (error != null) {
    ...
    private void printResult(RecognizerResult results) {
        ...
        mResultText.setText(resultBuilder.toString());

package com.simplation.android_banner;
...
import com.simplation.android_banner.helper.LoopScaleHelper;
...
import com.simplation.android_banner.listener.PageChangeListener;
...
public class ConvenientBanner<T> extends RelativeLayout {
    ...
    private LoopScaleHelper loopScaleHelper;
    ...
    public ConvenientBanner(Context context, AttributeSet attrs) {
        super(context, attrs);
        @SuppressLint("CustomViewStyleable") TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.Banner_Style);
        canLoop = a.getBoolean(R.styleable.Banner_Style_canLoop, true);
        autoTurningTime = a.getInteger(R.styleable.Banner_Style_autoTurningTime, -1);
        a.recycle();
        init(context);
    }
    ...
    private void init(Context context) {
        ...
        viewPager = hView.findViewById(R.id.loopViewPager);
        ...
        loopScaleHelper = new LoopScaleHelper();
    ...
    public ConvenientBanner setPages(ViewHolderCreator holderCreator, List<T> datas) {
        this.mDatas = datas;
        pageAdapter = new PageAdapter(holderCreator, mDatas, canLoop);
        viewPager.setAdapter(pageAdapter);

        if (page_indicatorId != null)
            setPageIndicator(page_indicatorId);

        loopScaleHelper.setFirstItemPos(canLoop ? mDatas.size() : 0);
        loopScaleHelper.attachToRecyclerView(viewPager);
    ...
    public void notifyDataSetChanged() {
        ...
        if (page_indicatorId != null)
            setPageIndicator(page_indicatorId);
        loopScaleHelper.setCurrentItem(canLoop ? mDatas.size() : 0);
    }
     ...
     * 底部指示器资源图片
     *
     * @param page_indicatorId imageResource
     */
    public ConvenientBanner setPageIndicator(int[] page_indicatorId) {
        ...
        for (int count = 0; count < mDatas.size(); count++) {
            ...
            if (loopScaleHelper.getFirstItemPos() % mDatas.size() == count)
        ...
        loopScaleHelper.setOnPageChangeListener(pageChangeListener);
    ...
    public ConvenientBanner setOnPageChangeListener(OnPageChangeListener onPageChangeListener) {
        ...
        if (pageChangeListener != null)
            pageChangeListener.setOnPageChangeListener(onPageChangeListener);
        else loopScaleHelper.setOnPageChangeListener(onPageChangeListener);
        return this;
    }
    ...
    public int getCurrentItem() {
        return loopScaleHelper.getRealCurrentItem();
    }
    ...
    public ConvenientBanner setCurrentItem(int position, boolean smoothScroll) {
        loopScaleHelper.setCurrentItem(canLoop ? mDatas.size() + position : position, smoothScroll);
        return this;
    }
    ...
    public ConvenientBanner setFirstItemPos(int position) {
        loopScaleHelper.setFirstItemPos(canLoop ? mDatas.size() + position : position);
        return this;
    }
    ...
    public ConvenientBanner setPageIndicatorAlign(PageIndicatorAlign align) {
        ...
        layoutParams.addRule(RelativeLayout.ALIGN_PARENT_LEFT, align ==
                PageIndicatorAlign.ALIGN_PARENT_LEFT ? RelativeLayout.TRUE : 0);

        layoutParams.addRule(RelativeLayout.ALIGN_PARENT_RIGHT, align ==
                PageIndicatorAlign.ALIGN_PARENT_RIGHT ? RelativeLayout.TRUE : 0);

        layoutParams.addRule(RelativeLayout.CENTER_HORIZONTAL, align ==
                PageIndicatorAlign.CENTER_HORIZONTAL ? RelativeLayout.TRUE : 0);
        ...
        public void run() {
            ...
            if (convenientBanner != null) {
                if (convenientBanner.viewPager != null && convenientBanner.turning) {
                    int page = convenientBanner.loopScaleHelper.getCurrentItem() + 1;
                    convenientBanner.loopScaleHelper.setCurrentItem(page, true);
